...
namespace common\modules\doc\services;
...
class Route extends Object
{
    ...
    public function getAppRoutes($module = null)
    {
        ...
        $cache->set('api_url', $result, 1*24*3600);
        return $result;
    }
    ...
    protected function getRouteRecrusive($module, &$result)
    {
        ...
        try {
            ...
            foreach ($module->getModules() as $id => $child) {
               if( in_array($id,['uploader','h5-upload','api']) ){
                   continue;
               }
            ...
            foreach ($class->getMethods() as $method) {
                ...
                if ($method->isPublic() && !$method->isStatic() && strpos($name, 'action') === 0 && $name !== 'actions') {
                    ...
                    foreach ($docCommentArr as $key=>$comment) {
                        ...
                        if(!$start && stripos($comment, '@example')){
                            $start = true;
                        }
                        if($start && !stripos($comment, '@example')){
                                $example .= '</br>'.$comment;
                            ...
                            if (preg_match('/\$[A-Z0-9]*/', @$paramCommentArr[1])) {
                                ...
                                if($method->getParameters()){
                                    ...
                                    if(isset($paramCommentArr[2])){
                                        ...
                                        $params['desc'] =  $paramCommentArr[2] ;

                                    }
                                    if(isset($paramCommentArr[3])){

                                        $params['require'] =  $paramCommentArr[3] ?  true : false;

                                    }
                                    if(isset($paramCommentArr[4])){

                                        $params['default'] =  $paramCommentArr[4] ;

                                    }

                                    if(isset($paramCommentArr[5])){

                                        $params['other'] =  $paramCommentArr[5] ;
                        ...
                        if (!isset($returnCommentArr[2])) {
                            ...
                            //兼容处理有空格的注释
                            $returnCommentArr[2] = implode('<br/>', array_slice($returnCommentArr, 2));
                        }
                    ...
                    $result[$id]['example'] = substr($example,0,strlen($example)-4);
                }
