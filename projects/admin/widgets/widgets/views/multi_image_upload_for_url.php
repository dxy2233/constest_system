<div style="float:left;display: none;border: 1px dashed gray; padding: 2px;margin-right:10px;margin-bottom: 10px;" id="attachement_template_<?php echo $this->context->id; ?>" class="form-inline upload_img_result">
    ...
    <input type="text" class="txt-url" name="url_<?php echo $this->context->name; ?>" style="width: 92px;height: 16px;padding: 0 2px"></p>
    <p style="line-height: 18px;overflow: hidden">
        <a href="" style="color:red;float: right;margin-right: 2px" onclick="deleteAttachement(this);return false;">删除</a>
...
<script>
    ...
    function uploadProject_<?php echo $this->context->id; ?>(data,diyUrls) {
        ...
        if (renderUI) {
            ...
            attachementItem.find('input').eq(1).attr('value', "<?php echo $this->context->name; ?>").val(data.diyUrl);
    ...
    (function loadAttachments_<?php echo $this->context->id; ?>() {
        ...
        var diyUrls = <?php echo json_encode($this->context->diyUrls); ?>;
        for (var index in attachments) {
            var data = {
                'url': attachments[index],
                'diyUrl' :diyUrls[index]
            };
