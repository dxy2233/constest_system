...
use admin\assets\DateRangePickerAsset;
...
HTML;
    ...
    protected function initRange()
    {
        ...
        if ($this->presetDropdown) {
            ...
            $this->pluginOptions['ranges'] = [
                ...
                Yii::t('app', "最近 {n} 日", ['n' => 7]) => [
                    "moment().startOf('day').subtract(6, 'days')",
                    "moment()"
                ],
                Yii::t('app', "最近 {n} 日", ['n' => 30]) => [
    ...
    public function registerAssets()
    {
        ...
        if (empty($this->callback)) {
            ...
            $setScript = sprintf('$("input[name=%s]").val(start.format("X"));$("input[name=%s]").val(end.format("X"));',
                $this->name . 'MinDate',
                $this->name . 'MaxDate'
            );
            $this->callback = "function(start,end,label){{$script}{$setScript}}";
        }
...
JS;
    ...
    {
        $extraHiddens = sprintf('<input type="hidden" name="%s" /><input type="hidden" name="%s"/>',
            $this->name . 'MinDate',
            $this->name . 'MaxDate'
        );
        $append = $this->_startInput . $this->_endInput;
        if (!$this->hideInput) {
            return $this->getInput('textInput') . $extraHiddens . $append;
        }
